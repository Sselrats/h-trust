...
- Tailwind CSS
- Framer Motion
- Google Gemini 2.5 Flash (`@google/generative-ai`) — Step 4 실시간 LLM
- Playwright (녹화 자동화)
...
- `app/api/pipeline/[step]/`: Step별 API Route (Step 4: Gemini 호출)
- `app/lib/prompts/step4.ts`: Step 4 LLM 프롬프트 빌더 & 응답 파서
- `app/lib/pipeline.ts`: 각 Step의 클라이언트 측 실행 로직
- `app/lib/types.ts`: 공유 타입 (`Step4Result`, `FallbackReason`)
- `docs/`: 기획/피드백/시나리오 원본
- `output/`: 최종 영상 산출물
- `scripts/`: 녹화/인코딩 스크립트
...
```
키가 없으면 Step 4는 정적 데이터로 fallback되며 ⚠ 배지가 표시됩니다.
...
Step 4 (TRUST Layer — 초안 생성)은 실제 LLM 호출로 동작합니다.

**데이터 흐름:**
```
page.tsx
  └─ runStep(4)            ← app/lib/pipeline.ts
       └─ POST /api/pipeline/4
            └─ buildStep4Prompt(scenario)   ← app/lib/prompts/step4.ts
            └─ Gemini 2.5 Flash 호출
            └─ parseStep4Response(text)     ← app/lib/prompts/step4.ts
            └─ Step4Result 반환
  └─ Step4TrustAgent1 렌더링
```

**프롬프트 위치:** `app/lib/prompts/step4.ts` — `buildStep4Prompt()` 함수

프롬프트 구성:
- 시나리오 유형 (`scenario.title`)
- 고객 문의 내용 (`scenario.userText`)
- Domain AI 분석 결과 (`scenario.domainFindings`)
- 지침: 단정적 표현 금지, 고객 친화적 문체, 법령 조항 1개 인용
- 응답 형식: JSON `{"draft": "...", "citation": "..."}`

**Fallback:** API 키 없음/오류 시 정적 데이터로 자동 전환, ⚠ 배지 표시.

## Notes
- Step 3/4/5는 AI thinking 상태를 명시적으로 보여줍니다.
- Step 4는 Gemini 실시간 호출, Step 3/5는 정적 시뮬레이션입니다.
